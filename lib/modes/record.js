...
function Record(logger, prismUtils, mockFilenameGenerator, mock) {
  ...
  function serializeResponse(prism, req, res, data, isBase64, endDate) {
    var response = {
      ...
      endDate: endDate,
      delay: endDate - req.startDate
    };

    if (req.body) {
      response.reqPayload = req.body;
    }
  ...
  this.handleResponse = function(req, res, prism) {
    uncompress(res, function(res, data, isBase64) {
      ...
      if (doRecord) {
        serializeResponse(prism, req, res, data, isBase64, Date.now());
