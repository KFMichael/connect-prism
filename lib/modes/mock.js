...
function Mock(logger, prismUtils, mockFilenameGenerator, responseDelay) {
  ...
  function getJsonRequest(prism) {
    var httpRequestFile = path.join(__dirname, '../../../../', prism.config.mocksPath, path.basename(prism.config.mocksPath) + '_HTTP.json');
    var buffer = fs.readFileSync(httpRequestFile);
    ...
    function handleMockRequest() {
      ...
      fs.exists(overridePath, function(overrideExists) {
        if (overrideExists) {
          ...
          fs.exists(pathMocked, function(mockExists) {
            if (mockExists) {
              delayResponse(pathMocked, prism, req, res);
              var result = req.url.replace(/"|\/|\_|\?|\<|\>|\\|\:|\*|\|/g, '_').replace(/[0-9]{13}/g, '');
              (!index[result] ? index[result] = 1 : index[result]++ );
              playbackJsonArray.push({ "url" : index[result] + '_' + req.method + result + '.json'});
            } else {
              write404HttpResponse(req, res, pathMocked);
              serializeEmptyMock(prism, req, pathMocked);
              logger.verboseLog('Returned 404 for: ' + req.url);
            }
          });
        }
      });
      var httpRequestFilePlayback = fs.writeFile(
          path.join(__dirname, '../../../../', prism.config.mocksPath, path.basename(prism.config.mocksPath) + '_HTTP_Mock.json'),
          JSON.stringify(playbackJsonArray)
      );
    }

    prismUtils.getBody(req, function() {
      handleMockRequest(req, res, prism);
        console.log(getJsonRequest(prism));
    });
  };
}
